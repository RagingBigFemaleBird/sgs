    ...
    public class QiLinGong : Weapon
    {
        ...
        public class QiLinGongSkill : TriggerSkill
        {
            protected void Run(Player Owner, GameEvent gameEvent, GameEventArgs eventArgs)
            {
                ...
                {
                    int answer = 0;
                    Game.CurrentGame.UiProxies[Owner].AskForMultipleChoice(new MultipleChoicePrompt("QiLinGong", this), new List<String> { Prompt.MultipleChoiceOptionPrefix + "JiaYiMa", Prompt.MultipleChoiceOptionPrefix + "JianYiMa" },
                        out answer);
                    if (answer == 1)
                    {
            ...
            public QiLinGongSkill()
            {
                var trigger = new AutoNotifyPassiveSkillTrigger(
                    this,
                    (p, e, a) => { return (a.Card.Type is Sha) && (Game.CurrentGame.Decks[a.Targets[0], DeckType.Equipment].Any(s => (s.Type is DefensiveHorse) || (s.Type is OffensiveHorse))); },
                    Run,
