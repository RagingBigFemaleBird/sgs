...
namespace Sanguosha.Expansions.Wind.Skills
{
    ...
    /// </summary>
    public class GuHuo : CardTransformSkill
    {
        ...
        protected override bool DoTransformSideEffect(CompositeCard card, object arg, List<Player> targets)
        {
            ...
            {
                int answer = 1;
                if (!Game.CurrentGame.UiProxies[player].AskForMultipleChoice(new MultipleChoicePrompt("GuHuo", Owner, (move.cards[0].AddtionalType).CardType), Prompt.YesNoChoices, out answer))
                {
            ...
            if (!guhuoSucceed)
            {
                if (Game.CurrentGame.Decks[null, DeckType.GuHuo][0].Type.GetType().IsAssignableFrom(Game.CurrentGame.Decks[null, DeckType.GuHuo][0].AddtionalType.GetType()))
                {
                    foreach (var player in toProcess)
                    {
                        if (believe[player] == 0)
